  ...
  async handleRideRequest(
    ...
    switch (selectedPaymentOption.type) {
      ...
      case 'card':
        ...
        const card = selectedPaymentOption.savedCard;
        if (!card) return { success: false, error: 'Inget kort valt' };
        // Ensure required fields are present
        const normalizedCard = {
          id: card.id,
          last4: card.last4,
          brand: card.brand,
          expiryMonth: card.expiryMonth ?? card.expMonth ?? 0,
          expiryYear: card.expiryYear ?? card.expYear ?? 0,
        };
