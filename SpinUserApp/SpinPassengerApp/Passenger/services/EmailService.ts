...
class EmailService {
    ...
    static async sendWelcomeEmail(
        ...
        try {
            const subject = accountType === 'driver'
                ? locale.startsWith('sv')
                    ? 'Välkommen som förare hos Spin 🚖'
                    : 'Welcome as a Spin Driver 🚖'
                : locale.startsWith('sv')
                    ? 'Välkommen till Spin 🟡'
                    : 'Welcome to Spin 🟡';

            const body = accountType === 'driver'
                ? locale.startsWith('sv')
                    ? `Hej ${name},

Tack för att du registrerade dig som förare hos SpinTaxi.

Så här kommer du igång:
1) Slutför förar-onboarding i appen (biluppgifter, dokument, profil).
2) Vänta på godkännande från vårt team.
3) När du är godkänd kan du gå online och ta emot körningar.

Behöver du hjälp? Svara på detta mail eller besök Hjälp & Support i appen.

Vänliga hälsningar,
SpinTaxi Teamet`
                    : `Hi ${name},

Thanks for signing up as a SpinTaxi driver.

Get started:
1) Complete driver onboarding in the app (car details, documents, profile).
2) Wait for our team's approval.
3) Once approved, you can go online and accept rides.

Need help? Reply to this email or visit Help & Support in the app.
...
✓ Få tillgång till alla förare-funktioner i appen
